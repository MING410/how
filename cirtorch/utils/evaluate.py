...
def compute_ap(ranks, nres):
    ...
    """
    # https://yongyuan.name/blog/evaluation-of-information-retrieval.html
...
def compute_map(ranks, gnd, kappas=[]):
    ...
    map = 0.
    nq = len(gnd['q']) # number of queries
    aps = np.zeros(nq)
    ...
    for i in np.arange(nq):
        ...
        #True になったのみを残る　
        #e.g. pos=[0,1,3,6]
        pos  = np.arange(ranks.shape[0])[np.in1d(ranks[:,i], qgnd['label'])]
