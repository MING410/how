...
class HOWNet(nn.Module):
    ...
    def copy_excluding_dim_reduction(self):
        ...
        # self.__class__将实例变量指向类
        # self.__class__.__name__ 再去调用__name__类属性
 ...
 # @classmethod也不需要self参数，但第一个参数需要是表示自身类的cls参数。
    ...
    def train(self, mode=True):
        ...
        # 大部分情况下可以理解为 需要继承父类并做出关联改变时 才需要用super
...
def extract_vectors_local(net, dataset, device, *, features_num, scales):
    ...
    net.eval()
    loader = torch.utils.data.DataLoader(dataset, shuffle=False, pin_memory=True, num_workers=6)
